...
function expandCollapseUtilities(cy) {
  ...
  return {
    ...
    expandNodeBaseFunction: async function (node, single, layoutBy) {
      ...
      var positionDiff = {
        x:
          ...
          node._private.data["position-before-collapse"].x,
        y:
          node._private.position.y -
          node._private.data["position-before-collapse"].y,
      };

      node.removeData("infoLabel");
      if (node.data("type") === "group") {
        node.toggleClass("expanded", true);
        node.toggleClass("collapsed", false);
      }
     ...
     */
    simpleExpandGivenNodes: async function (nodes, applyFishEyeViewToEachNode) {
      nodes.data("expand", true); // Mark that the nodes are still to be expanded
      var roots = elementUtilities.getTopMostNodes(nodes);
      for (var i = 0; i < roots.length; i++) {
        var root = roots[i];
        await this.expandTopDown(root, applyFishEyeViewToEachNode); // MODIFIED: await // For each root node expand top down
      }
      return nodes;
    },
    /*
    ...
    expandGivenNodes: async function (nodes, options) {
    ...
    collapseGivenNodes: async function (nodes, options) {
      /*
    ...
    expandTopDown: async function (root, applyFishEyeViewToEachNode) {
      ...
      for (var i = 0; i < children.length; i++) {
        var node = children[i];
        await this.expandTopDown(node, applyFishEyeViewToEachNode);
      }
    },
    ...
    expandNode: async function (
      ...
      var commonExpandOperation = async function (
        opNode,
        opApplyFishEyeView,
        opSingle,
        opAnimate,
        ...
        if (opApplyFishEyeView) {
          opNode._private.data["width-before-fisheye"] =
            opNode._private.data["size-before-collapse"].w;
          opNode._private.data["height-before-fisheye"] =
            opNode._private.data["size-before-collapse"].h;

          await self.fishEyeViewExpandGivenNode(
            opNode,
            opSingle,
            opNode,
            opAnimate,
      ...
      if (node._private.data.collapsedChildren != null) {
        ...
        if (applyFishEyeView && single) {
          ...
          var bottomRightPosition = this.convertToModelPosition({
            x: cy.width(),
            y: cy.height(),
          });
          var padding = 80;
          ...
          var nodeBB = {
            x1:
              node._private.position.x -
              node._private.data["size-before-collapse"].w / 2 -
              padding,
            x2:
              node._private.position.x +
              node._private.data["size-before-collapse"].w / 2 +
            ...
            y1:
              node._private.position.y -
              node._private.data["size-before-collapse"].h / 2 -
              padding,
            y2:
              node._private.position.y +
              node._private.data["size-before-collapse"].h / 2 +
          ...
          if (!boundingBoxUtilities.equalBoundingBoxes(unionBB, bb)) {
            ...
            if (animate) {
              // If viewport change should be animated
              await cy
                .animate(
                  {
                    pan: viewPort.pan,
    ...
    collapseNode: function (node) {
      if (node._private.data.collapsedChildren == null) {
        ...
        if (node.data("type") === "group") {
          node.toggleClass("expanded", false);
     ...
     */
    fishEyeViewExpandGivenNode: async function (
      node,
      single,
      nodeToExpand,
      animate,
      ...
      var d_x_left = Math.abs(
        (node._private.data["width-before-fisheye"] - node.outerWidth()) / 2
      );
      var d_x_right = Math.abs(
        (node._private.data["width-before-fisheye"] - node.outerWidth()) / 2
      );
      var d_y_upper = Math.abs(
        (node._private.data["height-before-fisheye"] - node.outerHeight()) / 2
      );
      var d_y_lower = Math.abs(
        (node._private.data["height-before-fisheye"] - node.outerHeight()) / 2
      );

      var abs_diff_on_x = Math.abs(
        node._private.data["x-before-fisheye"] - x_a
      );
      var abs_diff_on_y = Math.abs(
        node._private.data["y-before-fisheye"] - y_a
      );
      ...
      for (var i = 0; i < siblings.length; i++) {
        ...
        await this.fishEyeViewMoveNode(
          sibling,
          T_x,
          T_y,
          nodeToExpand,
      ...
      if (node.parent()[0] != null) {
        ...
        await this.fishEyeViewExpandGivenNode(
          node.parent()[0],
          single,
          nodeToExpand,
          animate,
     ...
     */
    fishEyeViewMoveNode: async function (
      node,
      T_x,
      T_y,
      nodeToExpand,
      ...
      if (childrenList.length == 0) {
        var newPosition = {
          x: node._private.position.x + T_x,
          y: node._private.position.y + T_y,
        };
        if (!single || !animate) {
          ...
          await node
            ...
            .then(async () => {
              ...
              if (
                self.animatedlyMovingNodeCount > 0 ||
                !nodeToExpand.hasClass("cy-expand-collapse-collapsed-node")
              ) {
                return;
              }
    ...
    removeChildren: function (node, root) {
      ...
      for (var i = 0; i < children.length; i++) {
        ...
        if (root._private.data.collapsedChildren == null) {
          ...
          root._private.data.collapsedChildren =
            root._private.data.collapsedChildren.union(removedChild);
        }
      }
    },
    ...
    barrowEdgesOfcollapsedChildren: function (node) {
      ...
      var edges = relatedNodes.edgesWith(
        cy.nodes().not(relatedNodes.union(node))
      );
    ...
    repairEdges: function (node) {
      var connectedMetaEdges = node.connectedEdges(
   ...
   and it is not the root itself*/
      ...
      for (i = 0; i < children.length; i++) {
        if (children[i].data("collapsedChildren")) {
          collapsedChildren = collapsedChildren.union(
            this.getCollapsedChildrenRecursively(children[i], collapsedChildren)
          );
        }
    ...
    collapseGivenEdges: function (edges, options) {
      ...
      if (options?.groupEdgesOfSameTypeOnCollapse) {
        edges.forEach(function (edge) {
          ...
          if (edgesToCollapse.hasOwnProperty(edgeType)) {
            edgesToCollapse[edgeType].edges =
              edgesToCollapse[edgeType].edges.add(edge);

            if (
              edgesToCollapse[edgeType].directionType == "unidirection" &&
              (edgesToCollapse[edgeType].source != edge.source().id() ||
                edgesToCollapse[edgeType].target != edge.target().id())
            ) {
        ...
        edgesToCollapse["unknown"] = {
          edges: edges,
          directionType: "unidirection",
          source: edges[0].source().id(),
          target: edges[0].target().id(),
        };
        for (var i = 0; i < edges.length; i++) {
          if (
            edgesToCollapse["unknown"].directionType == "unidirection" &&
            (edgesToCollapse["unknown"].source != edges[i].source().id() ||
              edgesToCollapse["unknown"].target != edges[i].target().id())
          ) {
      ...
      for (const edgeGroupType in edgesToCollapse) {
        ...
        result.oldEdges = result.oldEdges.add(
          edgesToCollapse[edgeGroupType].edges
        );
        var newEdge = {};
        newEdge.group = "edges";
        newEdge.data = {};
        ...
        newEdge.data.collapsedEdges = this.check4nestedCollapse(
        ...
        newEdge.data["directionType"] =
          edgesToCollapse[edgeGroupType].directionType;
